...
func _on_register_character(character):
	...
	score_by_character[character] = 0
	print("[GameManager] Registered character " + character.name)

func _on_character_dropped_palet(character, palet):
	score_by_character[character] += 1
	total_dropped_palets += 1
	print("[GameManager] Character " + character.name + " dropped palet " + palet.name +
		". New score for character: " + str(score_by_character[character]) + ". Total dropped: " + str(total_dropped_palets))
